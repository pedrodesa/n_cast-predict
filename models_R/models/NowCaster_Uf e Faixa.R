...
#1 - Importando bases de dados Covid e ajustando o banco
      ...
      #1.1 - Banco 01/08    
             ...
             #1.1.2 - Criando Faixas Etárias 01/08 
               conf_covid <- conf_covid %>%
...
#2 - Dividindo os bancos por agregação
     ...
     #2.1 - Dividindo o banco de 01/08
               
            #2.1.1 - UF
                     recortes_UF_b1 <- split(conf_covid, conf_covid$estadoIBGE)

            #2.1.2 - Faixa
                     recortes_Faixa_b1 <- split(conf_covid, conf_covid$Faixa) 
...
#3 - Modelagem nowcaster
     ...
     #3.2 - Nowcast UF (EstadoNotificacaoIBGE)  
            ...
            for(estado in names(recortes_UF_b1)) {
                                                  dados_estado <- recortes_UF_b1[[estado]]
     ...
     #3.3 - Nowcast Faixa
            ...
            for(Faixa in names(recortes_Faixa_b1)) {
                                                      dados_Faixa <- recortes_Faixa_b1[[Faixa]]
...
#4 - Ajustando Dados por SE
     ...
     #4.1 - Listas de Armazenamento
            ...
            #4.1.1 - UF
                     #4.1.1.1 - Banco 01/08
                                lista_series_semana_UF1 <- list() # Inicializar uma lista para armazenar os resultados do primeiro banco
                     #4.1.1.1 - Banco 14/08
                                lista_series_semana_UF2 <- list() # Inicializar uma lista para armazenar os resultados do primeiro banco
                     
            #4.1.2 - Faixa
                     #4.1.1.1 - Banco 01/08
                                lista_series_semana_Faixa1 <- list() # Inicializar uma lista para armazenar os resultados do primeiro banco
                     #4.1.1.1 - Banco 14/08
     ...
     #4.1 - Colocando em SE para as UF
            ...
            #4.1.1 - Banco 01/08
                     for (estado in names(recortes_UF_b1)) {
                                                              dados_estado_1 <- recortes_UF_b1[[estado]]
                                                              serie_semana_1 <- dados_estado_1 %>%
                                                                                ...
                                                                                group_by(dt_event) %>%
                                                                                count()
                                                              lista_series_semana_UF1[[estado]] <- serie_semana_1
                     }
     ...
     #4.2 - Colocando em SE para as Faixa
            ...
            #4.2.1 - Banco 01/08                     
                     for (Faixa in names(recortes_Faixa_b1)) {
                                                             dados_Faixa_1 <- recortes_Faixa_b1[[Faixa]]
                                                             serie_semana_1 <- dados_Faixa_1 %>%
                                                                               ...
                                                                               group_by(dt_event) %>%
                                                                               count()
                                                             lista_series_semana_Faixa1[[Faixa]] <- serie_semana_1
                                }
...
#5 - Plot
     ...
     #5.2 - Gerando Gráficos
            ...
            #5.2.1- UF
                    for (uf in names(lista_series_semana_UF1)) {
                                                                serie_uf <- lista_series_semana_UF1[[uf]]
                                                                ...
                                                                series_combinadas[[uf]] <- serie_combinada
                    ...
                    #5.2.2 - Faixa
                             for (Faixa in names(lista_series_semana_Faixa1)) {
                                                                               serie_fx <- lista_series_semana_Faixa1[[Faixa]]
